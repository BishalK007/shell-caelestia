...
import qs.modules.ephemera.port.Widgets
    ...
    Column {
        ...
        Repeater {
            ...
            Column {
                ...
                // ── Browser login (CLIProxyAPI OAuth) — alternative to an API key ──
                Row {
                    id: loginRow
                    width: parent.width
                    spacing: Theme.spacingS
                    visible: aiService.loginSupported(delegate.modelData.provider)

                    property bool isProxy: aiService.authModeFor(delegate.modelData.provider) === "proxy"
                    property bool loggingIn: aiService.proxyLoginProvider === delegate.modelData.provider

                    DankButton {
                        text: loginRow.loggingIn ? "Logging in…"
                              : (loginRow.isProxy ? "Re-login" : "Login with browser")
                        enabled: !loginRow.loggingIn
                        onClicked: aiService.loginViaProxy(delegate.modelData.provider)
                    }

                    DankButton {
                        text: "Use API key"
                        visible: loginRow.isProxy && !loginRow.loggingIn
                        onClicked: aiService.setAuthMode(delegate.modelData.provider, "apikey")
                    }

                    StyledText {
                        text: loginRow.loggingIn ? "Complete sign-in in your browser…"
                              : (loginRow.isProxy
                                 ? (aiService.proxyReady ? "Routing via login (proxy)" : "Login mode — starting proxy…")
                                 : "")
                        visible: text.length > 0
                        font.pixelSize: Theme.fontSizeSmall
                        color: Theme.surfaceVariantText
