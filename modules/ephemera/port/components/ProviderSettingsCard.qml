    ...
    Column {
        ...
        AccordionSection {
            ...
            DankDropdown {
                ...
                // DankDropdown imperatively assigns its own currentValue on
                // selection, which severs a plain `currentValue: aiService.model`
                // binding. A Binding element re-pushes the value so the display
                // stays in sync when the model changes externally (e.g. a provider
                // switch auto-selects a different model).
                Binding {
