...
Item {
    ...
    // Live model lists fetched per cloud provider (id arrays keyed by provider).
    // Ollama is handled separately by OllamaManager.
    ...
    readonly property var modelChoices: {
        if (provider === "ollama") {
            ...
            for (var i = 0; i < ollamaCount; i++)
                olist.push(availableModels.get(i).name);
            return olist;
        }
        var live = cloudModels[provider];
        if (live && live.length > 0)
            return live;
        // Offline / pre-fetch fallback: static seed list.
    ...
    // ── Auth mode: "apikey" (default) or "proxy" (CLIProxyAPI browser login) ──
    // Per-provider map, persisted as JSON. In proxy mode requests route through
    // the local CLIProxyAPI using the logged-in account instead of an API key.
    property var authModes: ({})
    readonly property bool proxyMode: (authModes[provider] || "apikey") === "proxy"
    readonly property alias proxyLoginProvider: cliProxy.loginProvider
    readonly property alias proxyReady: cliProxy.ready

    function authModeFor(prov) {
        return authModes[prov] || "apikey";
    }
    function loginSupported(prov) {
        return cliProxy.loginSupported(prov);
    }
    function setAuthMode(prov, mode) {
        var m = ({});
        for (var k in authModes)
            m[k] = authModes[k];
        m[prov] = mode;
        authModes = m;
        saveSettingValue("authModes", JSON.stringify(m));
        if (mode === "proxy")
            cliProxy.ensureRunning();
        if (prov === provider) {
            updateBaseUrl();
            fetchCloudModels();
        }
    }
    function loginViaProxy(prov) {
        setAuthMode(prov, "proxy");
        cliProxy.runLogin(prov);
    }
    // Whether the active model accepts temperature/sampling params. False for
    // Opus 4.7/4.8 and OpenAI reasoning models (where they're dropped/400).
    readonly property bool samplingSupported: Providers.modelCapabilities(provider, model).samplingAllowed
    ...
    Component.onCompleted: {
        loadSettings();
        loadChatHistory();
        // Only probe Ollama if it's the active provider — don't auto-start it on first launch.
        if (isOllama)
            ollamaManager.ping();
    ...
    onProviderChanged: {
        ...
        updateBaseUrl();
        // Immediately pick a valid model for the new provider so the picker never
        // shows a cross-provider model; the live fetch refines the list after.
        // Compute from `provider` directly — the derived `modelChoices` property
        // hasn't re-evaluated yet inside this handler (it would lag one switch).
        var live = cloudModels[provider];
        var choices = (live && live.length > 0) ? live : Providers.getModelList(provider);
        if (choices.length > 0 && choices.indexOf(model) === -1) {
            model = choices[0];
            saveSettingValue("model", model);
        }
    ...
    // Re-fetch live models when a key becomes available (keyring resolves async).
    onHasApiKeyChanged: if (hasApiKey)
        fetchCloudModels()

    // Fetch the active cloud provider's live model list (GET /v1/models etc.) and
    // merge into modelChoices. Ollama is handled by OllamaManager; the static seed
    // list is the offline fallback.
    property string _modelFetchProvider: ""
    property string _modelFetchParseAs: ""
    function fetchCloudModels() {
        if (provider === "ollama" || modelListProc.running)
            return;
        var cmd, parseAs;
        if (proxyMode) {
            // Discover models through the local proxy's OpenAI-style /v1/models.
            if (!cliProxy.proxyKey) {
                cliProxy.ensureRunning();
                return;
            }
            cmd = ["curl", "-sS", "--max-time", "15",
                "-H", "Authorization: Bearer " + cliProxy.proxyKey,
                cliProxy.baseUrl + "/v1/models"];
            parseAs = "openai";
        } else {
            var key = resolveApiKey();
            if (!key)
                return;
            cmd = Providers.listModelsCommand(provider, baseUrl, key);
            if (!cmd)
                return;
            parseAs = provider;
        }
        _modelFetchProvider = provider;
        _modelFetchParseAs = parseAs;
        modelListProc.command = cmd;
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                var prov = root._modelFetchProvider;
                var list = Providers.parseModelList(root._modelFetchParseAs || prov, text);
                if (!list || list.length === 0)
                    return;
                var next = ({});
                for (var k in root.cloudModels)
                    next[k] = root.cloudModels[k];
                next[prov] = list;
                root.cloudModels = next;
                // Auto-select a valid model if none chosen or the saved one is stale.
                if (prov === root.provider && (!root.model || list.indexOf(root.model) === -1)) {
                    root.model = list[0];
                    root.saveSettingValue("model", root.model);
                }
            }
        }
    }
    ...
    // Local CLIProxyAPI manager — powers the per-provider "Login" (OAuth) auth mode.
    CliProxyManager {
        id: cliProxy
    }
    ...
    Connections {
        ...
        // Refresh the model list once the proxy is reachable, or after a login
        // adds a new account (its models then appear on /v1/models).
        function onReadyChanged(): void {
            if (cliProxy.ready && root.proxyMode)
                root.fetchCloudModels();
        }
        function onLoginFinished(provider: string, ok: bool): void {
            if (ok && provider === root.provider)
                root.fetchCloudModels();
        }
    }
    ...
    function resolveApiKey() { return proxyMode ? cliProxy.proxyKey : keyringService.resolveApiKey(provider); }
    ...
    function loadSettings() {
        ...
        provider = String(PluginService.loadPluginData(pluginId, "provider", "anthropic")).trim() || "anthropic";
        ...
        try {
            authModes = JSON.parse(String(PluginService.loadPluginData(pluginId, "authModes", "{}"))) || ({});
        } catch (e) {
            authModes = ({});
        }
